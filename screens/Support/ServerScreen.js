...
export default function ServerScreen() {
    ...
    useEffect(() => (sensorId !== "" ? db.Sensors.listenOne(setSensor, sensorId) : undefined), [
        sensorId,
    ]);

    const [category, setCategory] = useState(null);
    useEffect(
        () => (sensor ? db.Categories.listenOne(setCategory, sensor.categoryId) : undefined),
        [sensor]
    );
    ...
    useEffect(
        () =>
            category && category.name === "Motion"
                ? db.Sensors.Readings.listen2OrderByWhen(setImages, sensor.id)
                : undefined,
        [category]
    );
        ...
        const getData = async () => {
            if (images.length >= 1) {
                setLatestImageURL(
                    await fb
                        .storage()
                        .ref(`sensors/${sensor.id}/images/${images[0].id}.jpg`)
                        .getDownloadURL()
                );
            }
        };
        ...
        const getData = async () => {
            if (images.length >= 2) {
                setPreviousImageURL(
                    await fb
                        .storage()
                        .ref(`sensors/${sensor.id}/images/${images[1].id}.jpg`)
                        .getDownloadURL()
                );
            }
        };
    ...
    return (
        <View style={styles.container}>
            ...
            >
                ...
                {sensors.map((sensor) => (
                    <Picker.Item key={sensor.id} label={sensor.id} value={sensor.id} />
                ))}
            </Picker>
            {sensor && category && category.name === "Motion" && (
